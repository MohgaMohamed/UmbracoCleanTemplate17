...
namespace MOHPortal.Core.Umbraco.Validation
{
    class ValidationHelper : IValidationHelper
    {
        ...
        public ValidationHelper(ValidationHelperSettings validationHelperSettings)
        {
            ...
            _urlRegex = new Regex(validationHelperSettings.UrlValidationRegex, RegexOptions.Compiled | RegexOptions.IgnoreCase);
        }
        ...
        public bool ValidateUrl(string url) => !string.IsNullOrWhiteSpace(url) && _urlRegex.IsMatch(url);
        public bool ValidateEgyptianNationalId(string value)
        {
            return !string.IsNullOrWhiteSpace(value) && value.Length == ValidationHelperSettings.EgyptianNationalIdLength && ValidateDigitsOnly(value);
        }
