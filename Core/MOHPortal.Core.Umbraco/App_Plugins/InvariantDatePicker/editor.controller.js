angular.module("umbraco").controller("InvariantDatePickerController", function ($scope, angularHelper, dateHelper, validationMessageService) {
    ...
    function inputChanged() {
        if ($scope.model.datetimePickerValue === "" && $scope.hasDatetimePickerValue) {
            ...
            var momentDate = moment($scope.model.datetimePickerInputValue, $scope.model.config.format, true).locale('en');
            if (!momentDate || !momentDate.isValid()) {
                momentDate = moment(new Date($scope.model.datetimePickerInputValue)).locale('en');
            }
            if (momentDate && momentDate.isValid()) {
                setDate(momentDate);
            }
    ...
    $scope.model.onValueChanged = function (newVal, oldVal) {
        if (newVal != oldVal) {
            ...
            var minDate = moment('0001-01-01').locale('en');
            var newDate = moment(newVal).locale('en');
    ...
    function updateModelValue(momentDate) {
        var curMoment = moment($scope.model.value).locale('en');
    ...
    function setDatePickerVal() {
        if ($scope.model.value) {
            ...
            if ($scope.model.value && Object.toBoolean($scope.model.config.offsetTime) && $scope.serverTimeNeedsOffsetting) {
                ...
                dateVal = $scope.model.value ? moment($scope.model.value, "YYYY-MM-DD HH:mm:ss").locale('en') : moment().locale('en');
            }
